function [ output_args ] = densetraj_encode_sge_2014_update( exp_ann, start_seg, end_seg )
...
%% kf_dir_name: name of keyframe folder, e.g. keyframe-60 for segment length of 60s   
	...
	medmd_file = sprintf('/net/per610a/export/das11f/plsang/trecvidmed13/metadata/medmd_2014_devel.mat');
	...
	test_clips = [MEDMD.RefTest.KINDREDTEST.clips, MEDMD.RefTest.MEDTEST.clips];
	
	train_clips = unique(train_clips);
	test_clips = unique(test_clips);
    ...
    for ii = start_seg:end_seg,
        ...
        video_file = sprintf('%s/%s/%s.mp4', video_dir, fileparts(MEDMD.lookup.(video_id)), video_id);
