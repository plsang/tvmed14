function [ output_args ] = densetraj_encode_ldc( exp_ann, start_seg, end_seg )
...
%% kf_dir_name: name of keyframe folder, e.g. keyframe-60 for segment length of 60s   
	...
	%medmd_file = '/net/per610a/export/das11f/plsang/trecvidmed13/metadata/medmd_2014_LDC2012E26.mat';
	...
	clips = MEDMD.(ldc_pat).PROGAll_PS.clips;
	[durations, sorted_clip_idx] = sort(MEDMD.(ldc_pat).PROGAll_PS.durations, 'descend');
    ...
    for ii = start_seg:end_seg,
		...
		video_file = fullfile(video_dir, ldc_pat, [video_id, '.mp4']);
		if ~exist(video_file, 'file'),
			msg = sprintf('Video not found <%s>\n', video_file);
			logmsg(logfile, msg);
			continue;
		end
		...
		output_file = sprintf('%s/%s/%s.mat', output_dir_fc, ldc_pat, video_id);
